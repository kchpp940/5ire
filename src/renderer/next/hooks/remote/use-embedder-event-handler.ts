...
export const useEmbedderEventHandler = (handler: EventHandler) => {
  const streamRef = useRef<Bridge.ReadableStreamProxy<Emitter.WildcardEventChunk<Embedder.Events>> | null>(null);
  ...
  useEffect(() => {
    ...
    let stream: Bridge.ReadableStreamProxy<Emitter.WildcardEventChunk<Embedder.Events>> | null = null;
    let cancelled = false;

    window.bridge.embedder.createEventStream().then(async (s) => {
      if (cancelled) {
        s.stop().catch(() => {});
        return;
      }
