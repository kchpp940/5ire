...
export const useUpdaterEventHandler = (handler: EventHandler) => {
  const streamRef = useRef<Bridge.ReadableStreamProxy<Emitter.WildcardEventChunk<Updater.Events>> | null>(null);
  ...
  useEffect(() => {
    ...
    let stream: Bridge.ReadableStreamProxy<Emitter.WildcardEventChunk<Updater.Events>> | null = null;
    let cancelled = false;

    window.bridge.updater.createEventStream().then(async (s) => {
      if (cancelled) {
        s.stop().catch(() => {});
        return;
      }
